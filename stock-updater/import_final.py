...
class M:
 def __init__(s,u):s.base_url=u;s.headers={};s.stats={"success":0,"errors":0,"skipped":0};s.collections_cache={};s.channel_id=None
 def login(s,e,p):
  try:
 ...
 def get_channel(s):
  try:
   r=requests.get(f"{s.base_url}/admin/sales-channels",headers=s.headers,timeout=10)
   if r.status_code==200:
    channels=r.json().get("sales_channels",[])
    if channels:s.channel_id=channels[0]["id"];print(f"  📺 Canal: {channels[0].get('name','default')}")
  except:pass
 def get_coll(s,n):
 ...
 def create(s,row):
  try:
   ...
   data={"title":title,"handle":handle,"status":"published","is_giftcard":False,"discountable":True,"options":[{"title":"Taille","values":["Default"]}],"variants":[{"title":"Default","sku":sku,"prices":[{"amount":int(price*100),"currency_code":"eur"}],"options":{"Taille":"Default"}}]}
   if cid:data["collection_id"]=cid
   r=requests.post(f"{s.base_url}/admin/products",headers=s.headers,json=data,timeout=30)
   if r.status_code in[200,201]:
    pid=r.json()["product"]["id"]
    if s.channel_id:
     r2=requests.post(f"{s.base_url}/admin/products/{pid}",headers=s.headers,json={"sales_channels":[{"id":s.channel_id}]},timeout=10)
    s.stats["success"]+=1;print(f"  ✅ {title[:40]}")
    return True
 ...
 def run(s,csv_file):
  s.get_channel();print(f"\n🚀 Import avec canal de vente\n")
